...
function renderPost() {
  ...
  const authorName = post.author?.nickname || post.authorName || "익명";
...
function renderComments() {
  ...
  if (list.length === 0) {
    ...
    list.forEach((c) => {
      // 예상 필드: id, content, createdAt, author: { id, nickname }
      const commentId = c.id;
      const content = c.content ?? "";
      const createdAt = c.createdAt || c.created_at;
      const authorName = c.author?.nickname || c.authorName || "익명";
  ...
  dom.commentsContainer.addEventListener("click", (e) => {
    ...
    if (target.classList.contains("c-delete")) {
      handleDeleteComment(commentId);
    } else if (target.classList.contains("c-edit")) {
